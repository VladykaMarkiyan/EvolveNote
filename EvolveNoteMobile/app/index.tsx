...
export default function LoginScreen() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const { access_token, user, is_first_login, has_goals } = data;
        
        await AsyncStorage.setItem('access_token', access_token);
        await AsyncStorage.setItem('username', user.username);

        if (is_first_login) {
          // Користувач щойно створив акаунт або вперше залогінився
          router.replace('/onboarding');
        } else if (!has_goals) {
          // Вже заходив раніше, але ще не створив жодної цілі
          router.replace('/firsthome');
        } else {
          // Постійний користувач із активними цілями
          router.replace('/home');
        }
      }
    } catch (error) {
